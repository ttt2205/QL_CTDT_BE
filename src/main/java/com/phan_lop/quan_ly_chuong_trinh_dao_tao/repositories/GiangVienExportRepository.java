...
public interface GiangVienExportRepository extends JpaRepository<GiangVien, Long> {
    ...
    @Query(value = """
        ...
        GROUP BY   
          hp.id, hp.ten_hoc_phan, hp.so_tin_chi, gv.id, gv.ten, gv.nam_sinh, gv.chuc_danh, gv.khoa, gv.trinh_do
        """, nativeQuery = true)
            
    List<GiangVienExportProjection> getExportByBoMon(@Param("boMon") String boMon);
}
